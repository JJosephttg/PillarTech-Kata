namespace CheckoutOrderTotalLib {
    public class GroceryPOSSystem {
        ...
        /// <summary> Adds specified identifier to scannable items using the base price </summary>
        /// <param name="itemId"> Grocery item identifier </param>
        /// <param name="price"> Base unit price </param>
        public void AddScannableItem(string itemId, double unitPrice) => _inventoryManager.AddProduct(itemId, unitPrice);

        #region Configuration
        /// <summary> Sets a markdown of an existing grocery item </summary>
        /// <param name="itemId"> Grocery item identifier </param>
        ...
        /// <exception cref="System.ArgumentException"> Thrown if item ID has not been configured yet via the AddScannableItem method </exception>
        public void SetMarkdown(string itemId, double markdown) => _inventoryManager.ConfigureMarkdown(itemId, markdown);

        /// <summary> Sets the special of an existing grocery item </summary>
        /// <param name="itemId"> Grocery item identifier </param>
        ...
        #region Scanning
        /// <summary> Adds grocery item and quantity to checkout </summary>
        /// <param name="itemId"> Grocery item identifier </param>
        /// <param name="weightOrQty"> Weight or quantity to add </param>
        /// <returns> True if item is scanned, and false if item is not a valid/configured grocery item. Items can be configured beforehand with the AddScannableItem method </returns>
        /// <exception cref="System.ArgumentOutOfRangeException"> Thrown if weight/quantity is 0/negative/nonfinite </exception>
        public bool ScanItem(string itemId, double weightOrQty = 1) => _inventoryManager.PerformWorkIfItemExists(itemId, gItem => _scanner.ScanItem(gItem, weightOrQty));

        /// <summary> Removes grocery item from checkout </summary>
        /// <param name="itemId"> Grocery item identifier </param>
        ...
        /// <summary> Calculates total pre-tax price of all current checkout items </summary>
